...
SENSOR_ATTRIBUTES = {
    "temperature": {"device_class": SensorDeviceClass.TEMPERATURE, "unit": "°C", "state_class": SensorStateClass.MEASUREMENT},
    "humidity": {"device_class": SensorDeviceClass.HUMIDITY, "unit": "%", "state_class": SensorStateClass.MEASUREMENT},
    "power": {"device_class": SensorDeviceClass.POWER, "unit": "W", "state_class": SensorStateClass.MEASUREMENT},
    "voltage": {"device_class": SensorDeviceClass.VOLTAGE, "unit": "V", "state_class": SensorStateClass.MEASUREMENT},
    "current": {"device_class": SensorDeviceClass.CURRENT, "unit": "A", "state_class": SensorStateClass.MEASUREMENT},
    "online": {"device_class": SensorDeviceClass.ENUM, "unit": None, "state_class": None},
}
...
async def async_setup_entry(
    ...
    for device in devices:
        ...
        sensor_attributes = [
            "temperature", "humidity", "power", "voltage", "current", "online"
        ]
        
        for attr_key in sensor_attributes:
            if any(a.get("key") == attr_key for a in attributes):
                entities.append(
                    SberSensor(coordinator, device_id, f"{name} {attr_key}", attr_key, device)
                )
...
class SberSensor(CoordinatorEntity, SensorEntity):
    ...
    def __init__(self, coordinator, device_id: str, name: str, attribute_key: str, device: dict):
        """Initialize sensor."""
    ...
    def native_value(self) -> Any:
        ...
        for state in reported:
            if state.get("key") == self._attribute_key:
                ...
                elif self._attribute_key in ["temperature", "humidity", "power", "voltage", "current"]:
